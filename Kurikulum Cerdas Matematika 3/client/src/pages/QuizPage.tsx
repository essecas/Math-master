...
import { generateQuestions, getLevelName, getGradeName } from '../lib/questions';
...
export function QuizPage({ grade, level, isRetry = false, onComplete, onBack }: QuizPageProps) {
  const [questions] = React.useState<Question[]>(() => generateQuestions(grade, level, isRetry));
        ...
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
            <div>
              ...
              {isRetry && (
                <p className="text-sm text-orange-600 mt-1 font-medium">Mode Ulang dengan Petunjuk Tambahan</p>
              )}
            </div>
                ...
                {currentQuestion.type === 'multiple-choice' && currentQuestion.options ? (
                  ...
                  <div className={shakeError ? 'shake-animation' : ''}>
                    <Input
                      ...
                      className="text-center text-xl h-14 rounded-xl border-2 font-semibold"
                    />
                  </div>
                )}
                ...
                {!isCorrect && (
                  <div className="bg-gradient-to-r from-amber-50 to-yellow-50 p-4 rounded-xl border-2 border-amber-200 hint-bounce-animation">
                    <p className="text-amber-900 text-base flex items-center gap-2 font-medium">
                      ...
                      <span>Tips: Perhatikan langkah-langkah di penjelasan ya!</span>
                    </p>
                  </div>
                )}
